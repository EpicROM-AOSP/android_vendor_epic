function __print_epic_functions_help() {
cat <<EOF
Additional EpicROM-AOSP functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- epicgerrit:   A Git wrapper that fetches/pushes patch from/to EpicROM-AOSP Gerrit Review.
- epicrebase:   Rebase a Gerrit change and push it again.
- epicremote:   Add git remote for EpicROM-AOSP Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cloremote:       Add git remote for matching CodeLinaro repository.
- githubremote:    Add git remote for EpicROM-AOSP Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the epic model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch epic_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/epic-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.epic.device | grep -q "$EPIC_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $EPIC_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function epicremote()
{
    ...
    then
        ...
        local PFX="EpicROM-AOSP/"
    else
        local PROJECT=$REMOTE
    fi

    local EPIC_USER=$(git config --get review.review.epicrom.org.username)
    if [ -z "$EPIC_USER" ]
    then
        git remote add epic ssh://review.epicrom.org:29418/$PFX$PROJECT
    else
        git remote add epic ssh://$EPIC_USER@review.epicrom.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'epic' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/EpicROM-AOSP/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.epic.device | grep -q "$EPIC_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        epicremote
        git push epic HEAD:refs/heads/'$1'
    fi
    '
}
...
function epicgerrit() {
    ...
    local user=`git config --get review.review.epicrom.org.username`
...
function epicrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "EpicROM-AOSP Gerrit Rebase Usage: "
        echo "      epicrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.epicrom.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.epic.device | grep -q "$EPIC_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/epic/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $EPIC_FIXUP_COMMON_OUT ]; then
